            ...
            if (data.button == PointerEventData.InputButton.Right)
            {
                ...
                else
                {
                    ...
                    if (eS != null)
                    {
                        ...
                        {
                            if (itemTypeOfSlot[i].Equals(item.TipoDoItem))
                            {
                            ...
                            for (int i = 0; i < eS.slotsInTotal; i++)
                            {
                                if (itemTypeOfSlot[i].Equals(item.TipoDoItem))
                                {
                                    if (eS.transform.GetChild(1).GetChild(i).childCount != 0)
                                    {
                                        ...
                                        if (item.TipoDoItem == ItemType.UFPS_Weapon)
                                        {
                                        ...
                                        {
                                            inventory.EquiptItem(item);
                                            if (item.TipoDoItem != ItemType.Backpack)
                                                inventory.UnEquipItem1(otherItemFromCharacterSystem.GetComponent<ItemOnObject>().item);
                                        }
                                        if (this == null)
                                        {
                ...
                if (!gearable && item.TipoDoItem != ItemType.UFPS_Ammo && item.TipoDoItem != ItemType.UFPS_Grenade)
                {
