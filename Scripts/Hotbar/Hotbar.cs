...
public class Hotbar : MonoBehaviour
{
    ...
    void Update()
    {
        for (int i = 0; i < slotsInTotal; i++)
        {
            ...
            {
                if (transform.GetChild(1).GetChild(i).childCount != 0 && transform.GetChild(1).GetChild(i).GetChild(0).GetComponent<ItemOnObject>().item.TipoDoItem != ItemType.UFPS_Ammo)
                {
